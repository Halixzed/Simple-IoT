...
def send_command(command):
    ...
    print('Server response (without decryption): ' + data)
    decrypted_data = client_cipher.decrypt(data)
    print("Server response: " + decrypted_data)
    messagebox.showinfo("Info: ",  str(decrypted_data))
...
def start_client():
    ...
    action_label = tk.Label(root, text="Choose action (REGISTER, LOGIN):")
    action_label.pack(pady=global_padding)
    action_label.pack()

    action_var = tk.StringVar()
    action_var.set("REGISTER")

    action_option = tk.OptionMenu(root, action_var, "REGISTER", "LOGIN")
    action_option.pack(pady=global_padding)
    action_option.pack()

    username_label = tk.Label(root, text="Enter username:")
    username_label.pack(pady=global_padding)
    username_label.pack()
    username_entry = tk.Entry(root)
    username_entry.pack(pady=global_padding)
    username_entry.pack()

    password_label = tk.Label(root, text="Enter password:")
    password_label.pack(pady=global_padding)
    password_label.pack()
    password_entry = tk.Entry(root, show="*")
    password_entry.pack(pady=global_padding)
    password_entry.pack()
    ...
    register_login_button.pack(pady=global_padding)
    register_login_button.pack()

    command_label = tk.Label(root, text="Enter command (ON, OFF, TEMP [0-100]):")
    command_label.pack(pady=global_padding)
    command_label.pack()
    command_entry = tk.Entry(root)
    command_entry.pack(pady=global_padding)
    command_entry.pack()
    ...
    command_button.pack(pady=global_padding)
    command_button.pack()
    ...
    quit_button.pack(pady=global_padding)
    quit_button.pack()
